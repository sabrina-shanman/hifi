...
FSTBaker::FSTBaker(const QUrl& inputMappingURL, const QString& bakedOutputDirectory, const QString& originalOutputDirectory, const QUrl& destinationPath, bool hasBeenBaked) :
        ModelBaker(inputMappingURL, bakedOutputDirectory, originalOutputDirectory, destinationPath, hasBeenBaked) {
    if (hasBeenBaked) {
...
void FSTBaker::bakeSourceCopy() {
    ...
    auto baker = getModelBakerWithOutputDirectories(bakeableModelURL, _bakedOutputDir, _originalOutputDir, _destinationPath);
