...
void FBXGeometry::computeShapeInfo(ShapeInfo& shapeInfo, const glm::vec3& dimensions, const FBXGeometry& geometry, const QUrl& modelURL, const glm::vec3& registrationPoint, const glm::vec3& modelOffset, const std::vector<glm::mat4>& jointTransforms) {
    ...
    if (type == SHAPE_TYPE_COMPOUND) {
        ...
        glm::vec3 scaleToFit = dimensions / geometry.getUnscaledMeshExtents().size();
        // multiply each point by scale before handing the point-set off to the physics engine.
        // also determine the extents of the collision model.
        glm::vec3 registrationOffset = dimensions * registrationPoint;
        for (int32_t i = 0; i < pointCollection.size(); i++) {
            for (int32_t j = 0; j < pointCollection[i].size(); j++) {
                // back compensate for registration so we can apply that offset to the shapeInfo later
                pointCollection[i][j] = scaleToFit * (pointCollection[i][j] + modelOffset) - registrationOffset;
            }
        }
        ...
        glm::mat4 invRegistraionOffset = glm::translate(dimensions * -registrationPoint);
        glm::mat4 identity;
        for (int i = 0; i < numFbxMeshes; i++) {
            const FBXMesh& mesh = geometry.meshes.at(i);
            if (mesh.clusters.size() > 0) {
                const FBXCluster& cluster = mesh.clusters.at(0);
                if (jointTransforms.size() < i) {
                    // we backtranslate by the registration offset so we can apply that offset to the shapeInfo later
                    localTransforms.push_back(invRegistraionOffset * jointTransforms[i] * cluster.inverseBindMatrix);
                }
                else {
                    localTransforms.push_back(invRegistraionOffset * identity * cluster.inverseBindMatrix);
                }
            } else {
        ...
        if (totalNumVertices > MAX_VERTICES_PER_STATIC_MESH) {
            qWarning() << "model" << modelURL << "has too many vertices" << totalNumVertices << "and will collide as a box.";
